...
> registration. Ownership *is* the account match. A robot that does not appear in a client's list
> has, in order of likelihood: signed in to a different account, lost its token, or gone offline.
...
| Storage | File, `0640`, root-owned, group-readable | `sessionStorage`, tab-scoped, plus a 24 h sliding idle window |
...
later by session restore — and stops punishing reloads. Storage is `sessionStorage`, not
`localStorage`: tab-scoped, dies with the tab.
...
**all of §5**, §6's token handling, and §7's protocol, idempotency rule and media replay.
...
| Client symptom | Robot-side cause, readable from `Status` |
|---|---|
